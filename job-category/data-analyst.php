...
<div class="center">
  ...
  <div class="jobs">
    <p class="title">Opportunities For Data Analyst</p>
  </div>
  <div class="parent">
  <?php
    $query="select * from jobs where employee_link=5";
    ...
    $query="select * from jobs where employee_link=5 order by id DESC limit $start_from,$num_per_page";
    $query_run=mysqli_query($con,$query);
  ...
  <div class="pagination">
    <p>See all jobs</p>
    <?php
        $sql="select * from jobs where employee_link=5";
        $rs_result=mysqli_query($con,$sql);
        $total_records=mysqli_num_rows($rs_result);
        $total_pages=ceil($total_records/$num_per_page);
        for($i=1;$i<=$total_pages;$i++){
        echo "<a href='data-analyst.php?page=".$i."'>".$i."</a>";
        }
    ?>
  </div>
